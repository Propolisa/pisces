...
import { UnderseaFogPluginMaterial } from "./materials.ts";
import {
    AssetsManager,
    Color3,
    Color4,
...
import { setupWater } from "./water";
import { boids3d } from "./external/BoidsWebGPU/src/main3d.ts";
import { PropFish } from "./creatures/propfish.ts";
...
export class Game {
    ...
    private _skipStages: boolean = false;
    debug_camera: FreeCamera;
    freeCamMode: boolean = false;
    player_camera: FreeCamera;
    private _fish: PropFish;
    constructor(canvas: HTMLCanvasElement, skipStages: boolean = false) {
        this._canvas = canvas;
        this._skipStages = skipStages; // Initialize skipStages flag
    ...
    private async _init(): Promise<void> {
        ...
        ShaderStore.IncludesShadersStoreWGSL.lightFragment = ShaderStore // todo remove hack
        ...
        window.addEventListener("keydown", (ev) => {
            //Shift+Ctrl+Alt+c
            if (ev.shiftKey && ev.ctrlKey && ev.altKey && ev.keyCode === 67) {
                if (this.freeCamMode) {
                    this.player_camera.detachControl(this._canvas);
                    this._scene.activeCamera = this.debug_camera;
                    this.debug_camera.attachControl(this._canvas, true);
                } else {
                    this.debug_camera.detachControl(this._canvas);
                    this._scene.activeCamera = this.player_camera;
                    this.player_camera.attachControl(this._canvas, true);
                }
    ...
    private async _main(): Promise<void> {
        ...
        let { renderLoopAction } = await boids3d(
            this._engine,
            this._scene,
            this.player_camera,
            { numBoids: 32 },
        ...
        this._engine.runRenderLoop(async () => {
            UnderseaFogPluginMaterial.fogCenter.x =
                this.player_camera.position.x;
            UnderseaFogPluginMaterial.fogCenter.y =
                this.player_camera.position.y;
            UnderseaFogPluginMaterial.fogCenter.z =
                this.player_camera.position.z;
            switch (this._state) {
                case State.START:
                    this._scene.render();
                    break;
    ...
    setupWater(scene: Scene, engine: Engine, light: DirectionalLight) {
        setupWater(scene, engine, light);
    }
    // goToStart
    private async _goToStart() {
        if (this._skipStages) {
            await this._setUpGame(); // Prepares the game scene
            await this._goToGame(); // Skip directly to the game if skipStages is true
            return;
        }
    ...
    private async _goToCutScene(): Promise<void> {
        ...
        const working_anim = new Image(
            "sparkLife",
            "./sprites/working_anim.png",
        );
        ...
        const dropoff_anim = new Image(
            "sparkLife",
            "./sprites/dropoff_anim.png",
        );
        ...
        const leaving_anim = new Image(
            "sparkLife",
            "./sprites/leaving_anim.png",
        );
        ...
        const reading_anim = new Image(
            "sparkLife",
            "./sprites/reading_anim.png",
        );
    ...
    private async _setUpGame() { //async
        ...
        RegisterMaterialPlugin("UnderseaFog", (material) => {
    ...
    private async _goToLose(): Promise<void> {
        ...
        this._scene.detachControl();
        let scene = new Scene(this._engine);
        scene.clearColor = new Color4(0.0, 0.39, 0.62, 1);
        let camera = new FreeCamera("camera1", new Vector3(0, 0, 0), scene);
        camera.setTarget(Vector3.Zero());
    ...
    private async _initializeGameAsync(scene): Promise<void> {
        ...
        this.debug_camera = new FreeCamera(
            "debug_camera",
            new Vector3(0, 0, 0),
            scene,
        );
        ...
        scene.onBeforeRenderObservable.add(() => {
            ...
            if (
                this._player.tutorial_move && this._player.tutorial_jump &&
        ...
        //     scene,
        // );
        // this.setupWater(scene, this._engine, sun);
        scene.clearColor = new Color4(0.0, 0.39, 0.62, 1);
